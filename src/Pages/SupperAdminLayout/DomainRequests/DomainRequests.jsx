...
const initialDomainRequests = [
  { id: "DOM-771", storeName: "Daily Bazar", requestedDomain: "dailybazar.com.bd", currentStatus: "Pending", dnsConfigured: "Yes", requestedDate: "3 hours ago", ipTarget: "76.76.21.21" },
  { id: "DOM-768", storeName: "Gadget Zone", requestedDomain: "gadgetzone.shop", currentStatus: "Pending", dnsConfigured: "No", requestedDate: "Yesterday", ipTarget: "76.76.21.21" },
  { id: "DOM-755", storeName: "Organic Foods", requestedDomain: "organicfoods.net", currentStatus: "Connected", dnsConfigured: "Yes", requestedDate: "4 days ago", ipTarget: "76.76.21.21" },
  { id: "DOM-740", storeName: "Fashion Craft", requestedDomain: "fashioncraft.xyz", currentStatus: "Rejected", dnsConfigured: "No", requestedDate: "1 week ago", ipTarget: "76.76.21.21" },
];
...
const DomainRequests = () => {
  ...
  const handleUpdateStatus = (id, newStatus, domainName) => {
    ...
    setRequests(prev =>
      prev.map(req => req.id === id ? { ...req, currentStatus: newStatus } : req)
    );
  };
      ...
      <div className="container mx-auto space-y-6">
        ...
        <div className="bg-slate-800/50 backdrop-blur-md p-6 rounded-xl border border-slate-700/50 shadow-xl flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-slate-400 mt-1">Manage custom domain mapping (CNAME/A Records) for enterprise vendor stores.</p>
          </div>
          <div className="bg-slate-900/50 px-4 py-2 rounded-lg border border-slate-700 text-sm font-semibold flex items-center gap-2">
            Server IP Target: <span className="text-cyan-400 font-mono">76.76.21.21</span>
          </div>
        </div>
                  ...
                  filteredRequests.map((req) => (
                    <tr key={req.id} className="hover:bg-slate-700/20 transition-colors">
                      <td className="py-4 px-5 font-semibold text-white">
                        {req.storeName}
                        <span className="block text-[10px] font-mono text-slate-500 mt-0.5">ID: {req.id}</span>
                      </td>

                      <td className="py-4 px-5 font-mono text-indigo-400 font-bold">
                        <a href={`https://${req.requestedDomain}`} target="_blank" rel="noreferrer" className="flex items-center gap-1 hover:underline">
                          {req.requestedDomain} <HiArrowSmRight className="text-slate-500" />
                        </a>
                      </td>

                      <td className="py-4 px-5">
                        <div className="flex items-center gap-2">
                          <span className={`w-3.5 h-3.5 rounded-full ${req.dnsConfigured === "Yes" ? "bg-emerald-500 animate-pulse" : "bg-rose-500"}`} />
                          <span className="text-xs text-slate-300">
                            {req.dnsConfigured === "Yes" ? `Pointed to ${req.ipTarget}` : "DNS Missing / Misconfigured"}
                          </span>
                        </div>
                      </td>
                      ...
                      <td className="py-4 px-5">
                        <span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold tracking-wide ${statusStyles[req.currentStatus]}`}>
                          {req.currentStatus}
                        </span>
                      </td>
                      <td className="py-4 px-5">
                        <div className="flex items-center justify-center gap-2">
                          <button 
                            onClick={() => toast.success(`Re-checking live DNS records for ${req.requestedDomain}...`)}
                          ...
                          {req.currentStatus === "Pending" && (
                            <>
                              <button 
                                onClick={() => handleUpdateStatus(req.id, "Connected", req.requestedDomain)}
                              ...
                              <button 
                                onClick={() => handleUpdateStatus(req.id, "Rejected", req.requestedDomain)}
        ...
        <div className="bg-slate-800/40 border border-slate-700/60 p-4 rounded-xl flex items-start gap-3">
          ...
          <div className="text-xs text-slate-400 space-y-1">
            <p className="font-bold text-slate-200 uppercase tracking-wider">Super Admin Technical Guide:</p>
            <p>For custom domains to work, vendors must configure their domain name registrar (Namecheap, GoDaddy) by adding an <span className="text-white font-mono bg-slate-900 px-1 py-0.5 rounded">A Record</span> pointing to your server IP <span className="text-cyan-400 font-mono">76.76.21.21</span>.</p>
            <p>Once DNS Check shows green, you can click <span className="text-white font-semibold">"Approve"</span> to inject the domain routing proxy in Vercel/Nginx reverse proxy server.</p>
          </div>
        </div>
