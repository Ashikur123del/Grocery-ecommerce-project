...
const initialPayouts = [
  { id: "POT-2026-883", vendor: "Apex Digital", amount: 45000, method: "Bkash (Merchant)", requestedAt: "12 Jan 2026", status: "Pending" },
  { id: "POT-2026-882", vendor: "Gadget BD", amount: 128000, method: "City Bank (BEFTN)", requestedAt: "28 Feb 2026", status: "Approved" },
  { id: "POT-2026-881", vendor: "StyleFest HQ", amount: 15500, method: "Nagad", requestedAt: "15 Mar 2026", status: "Rejected" },
];
      ...
      <div className="container mx-auto space-y-6">
        ...
        <div className="bg-slate-800/50 backdrop-blur-md p-6 rounded-xl border border-slate-700/50 shadow-xl flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-slate-400 mt-1">Review vendor disbursement requests, audit payout logs, and transition gateway statuses.</p>
          </div>
            ...
            <table className="w-full text-sm text-left border-collapse">
              <thead className="bg-slate-900/50 border-b border-slate-700/50 text-slate-400 uppercase text-[11px] font-bold tracking-wider">
                <tr>
                  ...
                  <th className="py-4 px-6 text-center w-[15%]">Gateway Status</th>
                      ...
                      >
                        ...
                        <td className="py-4 px-6 align-middle">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <div className="font-bold text-white text-base transition-colors">{payout.vendor}</div>
                              <span className="font-mono text-[10px] text-slate-500">Ref: {payout.id}</span>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="py-4 px-6 align-middle font-extrabold text-emerald-400 text-base">
                          <span className="inline-flex items-center gap-0.5">
                            <HiCash size={18} className="text-emerald-400 shrink-0" /> ৳{payout.amount.toLocaleString()}
                          </span>
                        </td>
                        ...
                        {/* 5. Inline Status Modifier Dropdown */}
                        <td className="py-4 px-6 align-middle text-center">
                          {isEditing ? (
                            ...
                            <span className={`px-2.5 py-0.5 rounded-md text-xs font-semibold tracking-wide border ${statusStyles[payout.status]}`}>
                        ...
                        {/* 6. Inline Row Action Controllers */}
                        <td className="py-4 px-6 align-middle text-center">
        ...
        <div className="bg-slate-800/40 p-4 rounded-xl border border-slate-700/40 flex items-start gap-3 shadow-inner">
          ...
          <div className="text-xs text-slate-400 space-y-1">
            <p className="font-bold text-slate-300 uppercase tracking-wider">Gateway Disbursement Protection Protocol:</p>
            <p>Moving a ledger line item to <span className="text-emerald-400 font-semibold">Approved</span> dispatches an API node to integrated digital wallet channels. Double-check destination merchant tokens prior to shifting states.</p>
          </div>
        </div>
